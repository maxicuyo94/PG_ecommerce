...
function App() {
  ...
  if (window.location.href.includes("approved")) {
    swal("Payment approved", "", "success")
      .then(resp => {
        if (resp) {
          dispatch(checkout(userLogedStorage.currentSession?.user.id, "approved"));
          history.push("/");
        }
      })

  } else if (window.location.href.includes("pending")) {
    swal("Payment pending", "", "warning")
      .then(resp => {
        if (resp) {
    ...
    swal("Payment rejected", "", "error")
      .then(resp => {
        if (resp) {
          dispatch(checkout(userLogedStorage.currentSession?.user.id, "rejected"));
          history.push("/");
        }
      })
  }
