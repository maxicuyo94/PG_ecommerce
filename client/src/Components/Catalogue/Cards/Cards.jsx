        ...
        return (
            <div className={dark ? styles.containerDark : styles.container}>
                ...
                <div className={styles.prices}>
                    ...
                    <div className={styles.price}><span>US$<b>{(props.price * (1 - props.discount / 100)).toFixed(2)}</b></span>
                    </div>
                </div>

                {cart ? <button className={styles.btnOff}
                    onClick={() => { cart && handleAddToCart(props) }}
                    onMouseEnter={() => setIsShown(true)}
                    onMouseLeave={() => setIsShown(false)}>
                    {isShown ? <><span> Add one more?</span><AddShoppingCartIcon style={{ fontSize: '1.5rem' }} /></>
                        : <><span> Added!</span><CheckCircleIcon style={{ fontSize: '1.25rem' }} /></>}
                </button>
                    :
